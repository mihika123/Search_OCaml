...
module type Set = sig
  ...
  (** [insert x s] is a set containing all the elements of [s] as well
      as element [x]. *)
  val insert : elt -> t -> t
  ...
  (** [difference] is set difference, that is, [difference s1 s2]
      contains exactly those elements that are elements of [s1] but not
      elements of [s2]. *)
  val difference : t -> t -> t

  (** [fold f init s] is [f xn (f ... (f x1 init) ...)], if [s] contains
      [x1]..[xn]. Elements are processed in order from least to
      greatest, where [x1] is the least element and [xn] is the
      greatest. *)
  val fold : (elt -> 'acc -> 'acc) -> 'acc -> t -> 'acc

  (** [to_list s] is a list containing the same elements as [s]. The
      order of elements in the list is in order from the least set
