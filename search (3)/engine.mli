...
(** An [Engine] indexes words found in text files and answers queries
    ...
    [\[("OCaml", \["a.txt"\]); ("is", \["a.txt"\]); ("BAE", \["a.txt";
    "b.txt"\]); ("salt", \["b.txt"\])\]] or
    [\[("OCaml", \["a.txt"\]); ("bae", \["b.txt"; "a.txt"\]); ("is",
    \["a.txt"\]); ("salt", \["b.txt"\])\]] or several other association
    lists, but none of the values could be "d/a.txt" or just "a". *)
